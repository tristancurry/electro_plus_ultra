...
public class electro_plus_ultra extends PApplet {
...
int chargeMass = 1;
float fieldConstantRadial = 0.40f;   //1 is strong enough to provoke very strong repulsion during rapid fire. 0.5 is good.
float fieldConstantUniform = 4000.00f;  //4000 works pretty well, 7000 is a bit strong!
...
int posM_col = color(255, 80, 180, 255);
int negM_col = color(0, 255, 255, 255);
int posF_col = color(255,0,0,255);
int negF_col = color(0,0,255,255);
...
//UPDATES
  ...
  voltageUseful = 1.f*round(1000*chargeDiff/capacitance)/1000;
  voltageVertical = 1.f*chargeDiffVert/capacitance;
  ...
  if(voltageUseful < 0){
    fill(1,1,1,50);
    text(str(1.f*voltageUseful),dropShadowOffset + mainArena.arenaWidth/2,dropShadowOffset + mainArena.arenaHeight/2);
  } else
  if(chargeDiff > 0){
    fill(1,1,1,50);
  ...
  voltageUseful < targetVoltage + tolerance*targetVoltage){
    ...
    //println(increment);
    timeAtTarget = 1.f*round(100*(timeAtTarget + increment))/100;
  ...
  if(voltageUseful < 0){
    text(str(1.f*voltageUseful),mainArena.arenaWidth/2, mainArena.arenaHeight/2);
  } else
  if(chargeDiff > 0){
...
//define and constrain 'aimPoint' for cannon display and shot trajectory
 aimPointX = (jamesCannon.xPos + 0.5f*jamesCannon.cannonWidth  + mainArena.xPos - mouseX)/(5 - 4*(abs(jamesCannon.xPos + 0.5f*jamesCannon.cannonWidth +mainArena.xPos-mouseX)/mainArena.arenaWidth));
 ...
 shotEnergy = (1.f*round(1000*(0.5f*chargeMass*aimPointMagSq/4000))/1000);
...
class Arena {
 ...
 //SETTERS
  public void set_Capacitance(float tempCapacitance){
    capacitance = tempCapacitance;
  }
...
Cannon() {
  ...
  lowerLimit = plate_R.yPos + plate_R.plateHeight - 0.5f*cannonWidth;
  upperLimit = plate_R.yPos + 0.5f*cannonWidth;
  speedLimit = 3;
  ySpeed = 0;
...
class Charge {
  ...
  public void move() {
    ...
    xPos = 1.f*xPos + xSpeed;    
    yPos = 1.f*yPos + ySpeed;
    xPos = constrain(xPos, -1, plate_R.xPos + 1);
    yPos = constrain(yPos, -1, plate_B.yPos + 1);
    ...
    if(player){
    timer ++;
    timerS = 1.f*round(100*timer/frameRate)/100;
    }
  ...
  public void display() {
    int chargeCol;
    int posCol = color(255,80,120,255);
    int negCol = color(0,255,255,255);
    int alphaInc = color(0,0,0,255/(1.f*trailLength));
    noStroke();
    if(sign < 0) {
      chargeCol = negM_col;
    } else if(sign > 0) {
...
public void plateCollisionCheck() {
    for(int i = 0; i < balloonList.length; i++){
...
public void chargeCollisionCheck(){
  for(int i=0; i < chargeList.size(); i++){
    Charge thisCharge = (Charge) chargeList.get(i);
    float kineticEnergy = 0.5f*thisCharge.mass*(sq(thisCharge.xSpeed) + sq(thisCharge.ySpeed));
          ...
          if(thisCharge.xPos >= plateArray[j].xPos && 
             ...
             if(kineticEnergy > ejectionThreshold){
               ...
               if(plateArray[j].numNeg_M > 0){
                ...
                float energyShare = kineticEnergy/(1.f*numNeg_Moved + numPos_Moved);
                  for(int k = 0; k < numPos_Moved; k++){
                    float newXSpeed;
                    float newYSpeed;
                     if(plateArray[j] == plate_T || plateArray[j] == plate_B){
                        newXSpeed = ejectionDamping*(thisCharge.xSpeed/abs(thisCharge.xSpeed))*random(-0.5f*sqrt(2*thisCharge.mass*(energyShare)), sqrt(2*thisCharge.mass*(energyShare)));
                     } else {
                        newXSpeed = -1*ejectionDamping*(thisCharge.xSpeed/abs(thisCharge.xSpeed))*random(-0.5f*sqrt(2*thisCharge.mass*(energyShare)), sqrt(2*thisCharge.mass*(energyShare)));
                     }
                     if(plateArray[j] == plate_R || plateArray[j] == plate_L){
                       newYSpeed = ejectionDamping*(thisCharge.ySpeed/abs(thisCharge.ySpeed))*sqrt(2*thisCharge.mass*(energyShare) - 0.5f*thisCharge.mass*sq(newXSpeed));
                     } else {
                       newYSpeed = -1*ejectionDamping*(thisCharge.ySpeed/abs(thisCharge.ySpeed))*sqrt(2*thisCharge.mass*(energyShare) - 0.5f*thisCharge.mass*sq(newXSpeed));
                     }
                   ...
                   for(int m = 0; m < numNeg_Moved; m++){ //if a collision happens with enough energy, eject moveable charges!
                    ...
                    float newYSpeed;
                     if(plateArray[j] == plate_T || plateArray[j] == plate_B){
                      newXSpeed = ejectionDamping*(thisCharge.xSpeed/abs(thisCharge.xSpeed))*random(-0.2f*sqrt(2*chargeMass*(energyShare)), sqrt(2*chargeMass*(energyShare)));
                    } else {
                      newXSpeed = -1*ejectionDamping*(thisCharge.xSpeed/abs(thisCharge.xSpeed))*random(-0.2f*sqrt(2*chargeMass*(energyShare)), sqrt(2*chargeMass*(energyShare)));
                    }
                    if(plateArray[j] == plate_R || plateArray[j] == plate_L){
                     newYSpeed = ejectionDamping*(thisCharge.ySpeed/abs(thisCharge.ySpeed))*sqrt(2*chargeMass*(energyShare) - 0.5f*thisCharge.mass*sq(newXSpeed));
                    } else {
                     newYSpeed = -1*ejectionDamping*(thisCharge.ySpeed/abs(thisCharge.ySpeed))*sqrt(2*chargeMass*(energyShare) - 0.5f*thisCharge.mass*sq(newXSpeed));
                    }
        ...
        Charge thisCharge = (Charge) chargeList.get(i);
          Balloon thisBalloon = balloonList[j];
            if(thisCharge.xPos < thisBalloon.xPos + 0.5f*thisBalloon.balloonWidth &&
               thisCharge.xPos > thisBalloon.xPos - 0.5f*thisBalloon.balloonWidth &&
               thisCharge.yPos < thisBalloon.yPos + 0.5f*thisBalloon.balloonWidth &&
               thisCharge.yPos > thisBalloon.yPos - 0.5f*thisBalloon.balloonWidth){
...
class ChargeBox {
  ...
  public void displayField(){
    ...
    pushMatrix();
    translate(xPos + 0.5f*resolution,yPos + 0.5f*resolution);
    noStroke();
    if(displayMode == 5){
    ...
    fill(255);
    ellipseMode(CENTER);
    triangle(-0.4f*resolution,0,-0.2f*resolution,0.2f*resolution,-0.2f*resolution,-0.2f*resolution);
    stroke(255);
    strokeWeight(1);
    line(-0.4f*resolution, 0,0.4f*resolution,0);
    popMatrix();
 ...
 public void updatePotential(){
  ...
  public void display(){
    ...
    fill(0.7f*adjV,0,0.7f*(255-adjV));
    rect(0,0,resolution,resolution);
...
class Plate {
  ...
  Plate(int tempXPos, int tempYPos, int tempWidth, int tempHeight, int tempNumPos_M, int tempNumPos_F, int tempNumNeg_M, int tempNumNeg_F, int tempControl) {
    ...
    squaresWide = round(1.f*plateWidth/plateSquareWidth);
    squaresHigh = round(1.f*plateHeight/plateSquareWidth);
  ...
  public void update() { //update values related to fixed and movable charges
    numPos = numPos_M + numPos_F;
    numNeg = numNeg_M + numNeg_F;
    charge = numPos - numNeg;
    charges = numPos + numNeg;
    ...
    posFactor = 0.5f + 0.5f*(1.f*charge/(totalCharges)); 
  ...
  public void display() { //draw the plate, colour according to charge
   ...
   for(int i = 0; i < plateWidth/5 ; i++){
      for(int j = 0; j < plateHeight/5; j++){
        ...
        if(fixedArray[i][j] == -2){
          ...
          int minceMeat =  color(posFactor*255, 0,(1-posFactor)*255);
          fill(minceMeat);
...
textAlign(LEFT);
 ...
 textAlign(RIGHT);
 text(str(1.f*round(capacitance*1.6f*1000)/1000)+ "x10^-19 F",0,0);
 textAlign(LEFT);
 translate(-1*(scoreWindough.inWidth - 20),20);
 text("scatter:",0,0);
 translate((scoreWindough.inWidth - 20),20);
 textAlign(RIGHT);
 text(str(1.f*(round(ejectionThreshold*1000)/1000)/4000)+ " eV",0,0);
 textAlign(LEFT);
  translate(-1*(scoreWindough.inWidth - 20),20);
  fill(posM_col);
 text("+ve ions:",0,0);
...
int currentStreak;
int bestStreak;
float targetVoltage = 0.25f;
float tolerance = 0.05f; //percent
float timeAtTarget;
...
scoreArray = PApplet.parseFloat(split(scoreString[0], ','));
...
class Windough {
  ...
  Windough(int tempOutWidth, int tempOutHeight, int tempInWidth, int tempInHeight, int tempWindowColour, int tempBorderColour){
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "electro_plus_ultra" };
    if (passedArgs != null) {
